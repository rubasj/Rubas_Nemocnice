...
public class ShiftGenerator {
    ...
    public NurseShift generateAfternoonShift(Date date) {
        ...
        if (cal.get(Calendar.DAY_OF_WEEK) >= 2 || cal.get(Calendar.DAY_OF_WEEK) >= 6)
            shift = new NurseShift("Odpoledni", Znalosti.ODPOLEDNI_SMENA, Znalosti.ODPOLEDNI_SMENA_OD, Znalosti.ODPOLEDNI_SMENA_DO, date, date);

        else
            shift = new NurseShift("Odpoledni vikend", Znalosti.ODPOLEDNI_SMENA_VIKEND, Znalosti.ODPOLEDNI_SMENA_VIKEND_OD, Znalosti.ODPOLEDNI_SMENA_VIKEND_DO, date, date);
